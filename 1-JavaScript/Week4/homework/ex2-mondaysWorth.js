...
function computeEarnings(tasks, rate) {
  ...
  const duration = tasks.map((task) => (task.duration * rate) / 60);

  const theSum = duration.reduce((accumulator, item) => {
    return accumulator + item;
  }, 0);

  const result = theSum.toFixed(2);
  return `€${result}`;
}
...
// example use case 2
const earnings1 = computeEarnings(
  [
    {
      name: 'walked 3 dogs',
      duration: 230,
...
console.log(`Total earnings: ${earnings1}`); // -> "Total earnings: €212.5"
